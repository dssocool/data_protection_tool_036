...
public static class AgentEndpoints
{
    ...
    {
        app.MapGet("/api/agents/{path}", async (string path, RpcAgentProxy agentProxy) =>
        {
            var (found, info) = await agentProxy.GetAgentInfoAsync(path);
        ...
        app.MapGet("/api/agents/{path}/user-id", async (string path, RpcAgentProxy agentProxy, ClientTableService clientTableService) =>
        {
            var (found, info) = await agentProxy.GetAgentInfoAsync(path);
            if (!found || info is null)
                return Results.NotFound(new { error = "Agent not found." });
        ...
        app.MapPost("/api/agents/{path}/validate-sql", async (string path, HttpRequest request, RpcAgentProxy agentProxy, ClientTableService clientTableService) =>
        {
        ...
        app.MapPost("/api/agents/{path}/save-connection", async (string path, HttpRequest request, RpcAgentProxy agentProxy, ClientTableService clientTableService) =>
        {
            var (found, info) = await agentProxy.GetAgentInfoAsync(path);
            if (!found || info is null)
                return Results.NotFound(new { error = "Agent not found." });
            ...
            try
            {
                ...
                var (connFound, connection) = await agentProxy.GetConnectionAsync(path);
                if (connFound && connection is not null)
                {
                    try
                    {
                        ...
                        await connection.PushConnectionsListAsync(connectionsJson);
                    }
                    catch
                    {
                    }
        ...
        app.MapPost("/api/agents/{path}/list-tables", async (string path, HttpRequest request, RpcAgentProxy agentProxy, ClientTableService clientTableService, ILogger<RpcAgentProxy> logger) =>
        {
            var (connFound, connection) = await agentProxy.GetConnectionAsync(path);
            if (!connFound || connection is null)
                return Results.NotFound(new { error = "Agent not found or not connected." });
        ...
        app.MapGet("/api/agents/{path}/list-schemas", async (string path, string rowKey, RpcAgentProxy agentProxy) =>
        {
            var (connFound, connection) = await agentProxy.GetConnectionAsync(path);
            if (!connFound || connection is null)
                return Results.NotFound(new { error = "Agent not found or not connected." });
        ...
        app.MapGet("/api/agents/{path}/connections", async (string path, RpcAgentProxy agentProxy, ClientTableService clientTableService) =>
        {
            var (found, info) = await agentProxy.GetAgentInfoAsync(path);
            if (!found || info is null)
                return Results.NotFound(new { error = "Agent not found." });
        ...
        app.MapPost("/api/agents/{path}/http-request", async (string path, HttpRequest request, RpcAgentProxy agentProxy) =>
        {
            var (connFound, connection) = await agentProxy.GetConnectionAsync(path);
            if (!connFound || connection is null)
                return Results.NotFound(new { error = "Agent not found or not connected." });
