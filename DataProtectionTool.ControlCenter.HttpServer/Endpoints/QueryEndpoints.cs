...
public static class QueryEndpoints
{
    ...
    {
        app.MapPost("/api/agents/{path}/validate-query", async (string path, HttpRequest request, RpcAgentProxy agentProxy, ClientTableService clientTableService) =>
        {
        ...
        app.MapPost("/api/agents/{path}/save-query", async (string path, HttpRequest request, RpcAgentProxy agentProxy, ClientTableService clientTableService) =>
        {
        ...
        app.MapPost("/api/agents/{path}/sample-query", async (string path, HttpRequest request, RpcAgentProxy agentProxy, ClientTableService clientTableService) =>
        {
            var (connFound, connection) = await agentProxy.GetConnectionAsync(path);
            if (!connFound || connection is null)
            {
        ...
        app.MapGet("/api/agents/{path}/queries", async (string path, string connectionRowKey, RpcAgentProxy agentProxy, ClientTableService clientTableService) =>
        {
            var (found, info) = await agentProxy.GetAgentInfoAsync(path);
            if (!found || info is null)
                return Results.NotFound(new { error = "Agent not found." });
