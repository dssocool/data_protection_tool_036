...
public static class TableEndpoints
{
    public static void MapTableEndpoints(this IEndpointRouteBuilder app)
    {
        app.MapPost("/api/agents/{path}/sample-table", async (string path, HttpRequest request,
            AgentRegistry registry, ClientTableService clientTableService,
            BlobServiceClient blobClient, BlobStorageConfig blobStorageConfig) =>
        {
            return await SampleTableCoreAsync(path, request, registry, clientTableService, blobClient, blobStorageConfig, useCache: true, labelPrefix: "Sample table");
        });
        ...
        app.MapPost("/api/agents/{path}/reload-sample-table", async (string path, HttpRequest request,
            ...
            BlobServiceClient blobClient, BlobStorageConfig blobStorageConfig) =>
        {
            return await SampleTableCoreAsync(path, request, registry, clientTableService, blobClient, blobStorageConfig, useCache: false, labelPrefix: "Reload sample table");
        });
    }
